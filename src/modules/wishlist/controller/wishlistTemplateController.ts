...
export class WishlistTemplateController {
	createWishlistTemplate = catchAsync(async (req: Request, res: Response) => {
		const { user } = req;
		const { name, description, emoji, colorTheme, items } = req.body;
	...
	getWishlistTemplates = catchAsync(async (req: Request, res: Response) => {
		...
		if (!user) {
			throw new AppError('Please login to view wishlist templates', 404);
		}

		const wishlistTemplates = await wishlistTemplateRepository.findAll();
		if (!wishlistTemplates) {
			throw new AppError('No wishlist templates found', 404);
		}
	...
	getWishlistTemplateById = catchAsync(async (req: Request, res: Response) => {
		const { user } = req;
		const { wishlistTemplateId } = req.query;

		if (!user) {
			throw new AppError('Please login to view wishlist templates', 404);
		}

		const wishlistTemplate = await wishlistTemplateRepository.findById(wishlistTemplateId as string);
		if (!wishlistTemplate) {
			throw new AppError('Wishlist Template not found', 404);
		}

		const wishlistTemplateIds = [wishlistTemplate.id];
		const items = await wishlistTemplateItemRepository.findBywishlistTemplateIds(wishlistTemplateIds);
		if (!items) {
			throw new AppError('No items found in these wishlist templates', 404);
		}
		...
		const wishlistTemplateWithItems = {
			wishlistTemplate: wishlistTemplate,
			items: itemsMap.get(wishlistTemplate.id) || [],
		};

		return AppResponse(res, 200, toJSON([wishlistTemplateWithItems]), 'Wishlist template fetched successfully');
	});
}
